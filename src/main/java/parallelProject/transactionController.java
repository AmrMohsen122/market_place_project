...
public class transactionController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        transactionTree.setRoot(Users);
        String branchU = "User ";
        String branchO = "Order ";
        String branchI = "item ";

        for (int k = 0; k < allUsers.size(); k++) {
            TreeItem<String> branchU1 = new TreeItem<>(branchU + (Integer.toString(k + 1) + "  " + (allUsers.get(k).getUsername()).toString()));
            Users.getChildren().addAll(branchU1);

            TreeItem<String> ORDER = new TreeItem<>("Orders:");
            Customer showCustomerTransaction = allUsers.get(k);
            branchU1.getChildren().addAll(ORDER);
